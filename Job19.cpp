...
int dp_dfs[10000]; // represents the distance from target to node i
    ...
    if(path[root]) {
        ...
        for(int j=0; j<graph[root].size(); j++) {
            ...
            if(!explored[graph[root][j]]) {
                dp_dfs[graph[root][j]] = dp_dfs[root] + 1;
...
int main()
{
    ...
    //Find farthest point from point 0
    BFS(0);

    int longestDST=0;
    int longestDSTI = 0;
    for(int i=0; i<n; i++) {
        if(dp_bfs[i] > longestDST) {
            longestDST = dp_bfs[i];
            longestDSTI = i;
        }
    }

    cout << endl << longestDST << endl;
    //re-check from the farthest point
    BFS(longestDSTI);
    longestDST = 0;
    int longestDSTJ = 0;
    for(int i=0; i<n; i++) {
    ...
    cout << "Longest possible distance is: " << longestDST << " - Between node " << longestDSTI << " and node " << longestDSTJ;
